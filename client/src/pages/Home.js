...
let socket;
const CONNECTION_PORT = "192.168.29.117:8007/";
...
function Home({ ID }) {
  ...
  const [sendermsg, setSenderMsg] = useState(null);

  const [list, setList] = useState({ addmsg: [], receivemsg: [] });
    ...
    return axios({
      ...
      .then((response) => {
        if (response.data) {
          ...
          response.data.profile = `${CONSTANTS.API_URL}uploads/${response.data.profile}`;
  ...
  useEffect(() => {
    if (receivemsg?.message) {
      setMessageList((prevItems) => [...prevItems, receivemsg]);
      setReceiveMsg(null);
    }

    if (sendermsg?.message) {
      // setMessageList((prevItems) => [...prevItems, sendermsg]);
      setSenderMsg(null);
    }
  }, [receivemsg, sendermsg]);
  ...
  const sendMessage = async () => {
    ...
    setSenderMsg(messageContent.content);
    // setMessage("");
      ...
      {userData && (
        <>
          <div style={{ display: "flex" }}>
            <img
              src={userData.profile ? userData.profile : avatar6}
              alt="img"
              style={{
                height: "4rem",
                width: "4rem",
            ...
            <div className="messages">
              {oldMessage.map((oldmes) => (
                <div
                  ...
                  key={oldmes._id}
                  id={oldmes.senderId == UserId ? "You" : "Other"}
                >
                  {/* <h1>{oldmes.text}</h1> */}
              ...
              {messageList.length != 0 &&
                messageList.map((val) => {
                  return (
                    <div
                      className="messageContainer"
                      id={val.author == UserId ? "You" : "Other"}
                      key={val}
                    >
